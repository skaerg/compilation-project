...
module InstructionSelector : InstructionSelector =
  struct
    ...
    let bin ins ~dst ~srcl ~srcr =
      (* Move left operand to register %R15 *)
      let i1 = Instruction ( movq ~src:srcl ~dst:(`Reg (X86_64_Architecture.R15)) ) in
      (* Apply the binary operation *)
      let i2 = Instruction ( ins ~src:srcr ~dst:(`Reg (X86_64_Architecture.R15))) in
      (* Store the result in dst *)
      let i3 = Instruction ( movq ~src:(`Reg (X86_64_Architecture.R15)) ~dst:dst ) in
    ...
    let div ~dst ~srcl ~srcr =
      ...
      let i1 = Instruction ( movq ~src:(`Imm (Lit (Mint.zero))) ~dst:(`Reg (X86_64_Architecture.RDX)) ) in
      let i2 = Instruction ( movq ~src:srcl ~dst:(`Reg (X86_64_Architecture.RAX)) ) in
      let i3 = Instruction ( movq ~src:srcr ~dst:(`Reg (X86_64_Architecture.R15)) ) in
      let i4 = Instruction ( idivq ~src:(`Reg (X86_64_Architecture.R15)) ) in
      let i5 = Instruction ( movq ~src:(`Reg (X86_64_Architecture.RDX)) ~dst:dst ) in
    ...
    let conditional_jump ~cc ~srcl ~srcr ~ll ~lr =
      ...
      (* Test whether [srcl, srcr] satisfies the relation described by [cc] *)
      let i1 =
        match (srcl, srcr) with
        | (`Reg reg, _)     -> [Instruction (cmpq ~src1:srcr ~src2:srcl)]
        | (`Imm _, `Imm _)  ->
          let i11 = Instruction (movq ~src:srcl ~dst:(`Reg (X86_64_Architecture.R15)) ) in
          let i12 = Instruction (cmpq ~src1:srcr ~src2:(`Reg (X86_64_Architecture.R15))) in
          [i11;i12]
        | (_,_)             -> [Instruction (cmpq ~src1:srcl ~src2:srcr)]
      in

      (* Jump to label ll if relation cc is verified *)
      let i2 = Instruction (jccl ~cc:cc ~tgt:ll) in

      (* Jump to label lr otherwise *)
      let i3 = Instruction (jmpl ~tgt:lr) in

      i1@[i2;i3]

    let switch ?default ~discriminant ~cases () =

      let rec instr_cases cases ind acc =
        match cases with
        ...
        | []    -> 
          ...
          | None    -> List.rev acc
          | Some l  -> 
            let default_instr = Instruction (jmpl ~tgt:l) in
            List.rev (default_instr::acc))

        | x::tl -> 

          let ind_instr = Instruction (movq 
            ~src:(`Imm (Lit (Mint.of_int (ind)))) 
            ~dst:(`Reg (X86_64_Architecture.R15)) ) in

          let cmp_instr = Instruction (cmpq 
            ~src1:discriminant
            ~src2:(`Reg (X86_64_Architecture.R15))) in

          let jmp_instr = Instruction (jccl ~cc:T.E ~tgt:x) in
          instr_cases (tl) (ind+1) (jmp_instr::cmp_instr::ind_instr::acc)

      in

      instr_cases (Array.to_list cases) 0 []
...
module FrameManager(IS : InstructionSelector) : FrameManager =
  struct
    ...
    let frame_descriptor ~params ~locals =
      ...
      (* 8 bytes i.e. 64 bits for each local variable *)
      let locals_space = (List.length locals) * 8 in

      let rec fill_stack_map map ls ind =
        match ls with
        | []    -> map
        | x::tl -> 
          let map2 = S.IdMap.add (x) (Mint.of_int (-ind * 8)) (map) in
          fill_stack_map map2 tl (ind+1)
      in

      let stack_map = fill_stack_map (S.IdMap.empty) (locals) (1) in

      { param_count = param_count; 
        locals_space = locals_space; 
        stack_map = stack_map; }

    let location_of fd id =

      let map = fd.stack_map in
      let local_var = S.IdMap.find_opt (id) (map) in

      match local_var with 
      | Some slot -> (X86_64_AST.addr ~offset:(Lit slot) ~scale:`One ~base:(X86_64_Architecture.RBP)) ()
      | None -> 
        let s = match id with S.Id (s) -> s in
        (X86_64_AST.addr ~offset:(Lab s) ~scale:`One ~base:(X86_64_Architecture.RIP)) ()

    let function_prologue fd =
      let i1 = T.Instruction (T.pushq ~src:(`Reg X86_64_Architecture.RBP)) in
      let i2 = T.Instruction (T.movq ~src:(`Reg X86_64_Architecture.RSP) ~dst:(`Reg X86_64_Architecture.RBP)) in
      let i3 = T.Instruction (T.subq ~src:(`Imm (Lit (Mint.of_int (fd.locals_space)))) ~dst:(`Reg X86_64_Architecture.RSP)) in
      
      [i1;i2;i3]

    let function_epilogue fd =
      let i1 = T.Instruction (T.addq ~src:(`Imm (Lit (Mint.of_int (fd.locals_space)))) ~dst:(`Reg X86_64_Architecture.RSP)) in
      let i2 = T.Instruction (T.popq ~dst:(`Reg X86_64_Architecture.RBP)) in
      let i3 = T.Instruction (T.Ret) in

      [i1;i2;i3]

    let call fd ~kind ~f ~args =

      let call_instr = T.Instruction (T.calldi ~tgt:f) in
      let align = T.Directive (PadToAlign { pow = 3; fill = 0x90; }) in

      let rec aux args acc = 
        match args with
        | [] -> acc @ align::[call_instr]
        | src::tl -> 
          let instr = T.Instruction (T.pushq ~src:src) in
          aux (tl) (instr::acc)
      in
